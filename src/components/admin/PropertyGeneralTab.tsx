...
export function PropertyGeneralTab({
  ...
  const renderPillToggle = (name: string, labelKey: string, Icon: typeof Star) => {
    const checked = getValue(name) === "true";
    return (
      <div className="flex items-end">
        <AdminPillToggle checked={checked} onChange={() => setBoolean(name, !checked)} icon={Icon}>
          {t(labelKey)}
        </AdminPillToggle>
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2" role="tabpanel">
      ...
      {renderPillToggle("is_featured", "Fields.is_featured", Star)}
